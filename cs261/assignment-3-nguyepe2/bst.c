...
int bst_size(struct bst* bst) {
   ...
   int euler_tour(struct bst_node* curr) {
      if(curr!=NULL) {
	 num++;
	 euler_tour(curr->left);
	 euler_tour(curr->right);
      } 
   }
   euler_tour(curr);
...
int bst_height(struct bst* bst) {
   ...
   int get_height(struct bst_node* curr) {
      int lefth, righth;
      if(curr==NULL)
	 return -1;
      lefth=get_height(curr->left);
      righth=get_height(curr->right);
      height=largest(lefth,righth) +1;
      return height;
   }
...
int bst_path_sum(int sum, struct bst* bst) {
   ...
   int get_total(struct bst_node* curr, int sum) {
      if(curr==NULL)
	 return 0;
      else {
	 int test=0;
	 int num=sum-curr->val;
	 if(num==0 && curr->left==NULL && curr->right==NULL)
	    return 1;
	 if(curr->left)
	    test=test || get_total(curr->left,num);
	 if(curr->right)
	    test=test || get_total(curr->right,num);
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
   struct bst_iterator* stack=malloc(sizeof(struct bst_iterator));
   ...
   do{
      if(!stack_isempty(s1) && curr==stack_top(s1)) { 
     // if(curr==stack_top(s1)) {
	    num=stack_pop(s1);
	    stack_push(s2,num);
	    curr=curr->right; 
	 }
